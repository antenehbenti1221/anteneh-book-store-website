(function(){
  ...
  function enhanceFree(){document.querySelectorAll('#freeGrid .book').forEach(card=>{const title=card.querySelector('.title'),preview=card.querySelector('.book-preview');if(!title||!preview||title.dataset.freeUiReady)return;title.dataset.freeUiReady='1';const old=card.querySelector('[data-card-view]');if(old){old.style.display='inline-flex';old.classList.add('book-view-button');old.textContent='📖 Read / Listen Free';old.onclick=e=>{e.preventDefault();e.stopPropagation();openFreeInline(card,preview,old)};preview.appendChild(old)}title.setAttribute('role','button');title.setAttribute('tabindex','0');const toggle=()=>{document.querySelectorAll('#freeGrid .book.expanded').forEach(other=>{if(other!==card){other.classList.remove('expanded');const p=other.querySelector('.book-preview');if(p)p.hidden=true}});const open=card.classList.toggle('expanded');preview.hidden=!open};title.onclick=e=>{e.preventDefault();e.stopPropagation();toggle()};title.onkeydown=e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();toggle()}}})}
  async function openFreeInline(card,preview,button){const b=typeof books!=='undefined'?books.find(x=>String(x.id)===String(button.dataset.id)):null;if(!b)return;let result=preview.querySelector('.free-access-result');if(!result){result=document.createElement('div');result.className='free-access-result';preview.appendChild(result)}if(result.dataset.ready==='1')return;button.disabled=true;button.textContent='Preparing…';result.hidden=false;result.innerHTML='<p class="small-note">Preparing your free book…</p>';try{const{data,error}=await client.rpc('create_store_order',{p_book_id:b.id,p_customer_name:'Free reader',p_customer_email:null,p_customer_phone:null});if(error)throw error;const o=Array.isArray(data)?data[0]:data;const kind=String(b.type||'').toLowerCase().includes('audio')?'audio':'ebook';const r=await fetch(`${C.SUPABASE_URL}/functions/v1/deliver-purchase`,{method:'POST',headers:{'Content-Type':'application/json',apikey:C.SUPABASE_PUBLISHABLE_KEY},body:JSON.stringify({access_token:o.access_token,kind})});const j=await r.json();if(!r.ok||!j.url)throw new Error(j.error||'Secure access link was not returned');result.dataset.ready='1';result.innerHTML=`<a class="btn primary" href="${String(j.url).replace(/&/g,'&amp;').replace(/\"/g,'&quot;')}" target="_blank" rel="noopener">${kind==='audio'?'🎧 Open audiobook':'📖 Open ebook'}</a>`;button.textContent='Ready'}catch(e){result.innerHTML='<p class="small-note">Could not open this book.</p><button class="btn" type="button" data-free-retry>Try again</button>';result.querySelector('[data-free-retry]').onclick=()=>openFreeInline(card,preview,button);button.disabled=false;button.textContent='📖 Read / Listen Free'}}
  function enhance(){enhanceGrid();enhanceFree()}
  const observer=new MutationObserver(enhance);observer.observe(document.getElementById('grid')||document.body,{childList:true,subtree:true});observer.observe(document.getElementById('freeGrid')||document.body,{childList:true,subtree:true});enhance();
  const fix=document.createElement('script');fix.src='free-books-fix.js?v=20260824-2';document.body.appendChild(fix);
})();
